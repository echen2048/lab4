...
loo   CMP	R2,#0
	  ...
	  ADD	R0,#1
	  SUB	R2,#1
	  B		loo ;fill databuffer with 0xFF

done  LDR	R0,=TimeBuffer
	  LDR	R1,=0xFFFFFFFF
	  MOV	R2,#100
lo	  CMP	R2,#0
	  BEQ	fin 
	  ...
	  ADD	R0,#4
	  SUB	R2,#1
	  B		lo ;fill timebuffer with 0xFFFFFFFF
	  
fin	  LDR	R0,=DataPt
	  LDR	R1,=DataBuffer 
	  STR	R1,[R0] ;set the datapt
	  LDR	R0,=TimePt
	  LDR	R1,=TimeBuffer
	  STR	R1,[R0] ;set the timept
	  
	  BL SysTick_Init
	  POP {R0-R4,PC}
	  BX LR
	  
;Debug capture has 31 lines, approx. 62 instruction cycles, requiring approx. 775ns. 
;the program has approx. 3.2e6 lines to capture, approx. 6.4e6 inst cycles and approx. 80ms. 
;the overhead with an 80MHz clock is therefore 0.000971%
Debug_Capture 
      PUSH {R0-R10,LR}
	  LDR R7,=DataPt
	  LDR R4,[R7] ;R4 is contents of DataPt
	  LDR R8,=TimePt
	  LDR R5,[R8] ;R5 is contents of TimePt
	  LDR R9, =prevtime
	  LDR R10,[R9]
	  
	  ;begin check to see if databuffer is full
	  LDR R6,=DataBuffer
	  ADD R6,R6,#100
	  CMP R4,R6
	  BEQ full ;check if databuffer full
	  	  	  	  
	  LDR R6,=GPIO_PORTA_DATA_R
	  LDRB R0,[R6]
	  AND R0,#0x10 ;get only bit 4 of port a data
	  
	  LDR R6,=GPIO_PORTE_DATA_R
	  LDRB R1,[R6]
	  AND R1,#0x0F ;only bits 0-3 of port e
	  ORR R0,R0,R1 ;combine them
	  STRB R0,[R4] ;store to datapointer address
	  ADD R4,#1 ;incr datapointer
	  STR R4,[R7] ;store datapointer new value

	  LDR R6,=NVIC_ST_CURRENT_R
	  LDR R2,[R6] ;R2 has systick current value
	  
	  LDR R3,=0x00FFFFFF
	  SUB R1,R10,R2 ;find elapse time
	  AND R1,R1,R3
	  
	  STR R1,[R5] ;store elapse to timebuffer
	  ADD R5,R5,#4 ;advance timepointer to next
	  STR R5,[R8] ;store back timepointer
	  STR R2,[R9] ;current time to previous time
